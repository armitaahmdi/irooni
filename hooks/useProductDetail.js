...
export function useProductDetail(product, routeParams) {
  ...
  const handleAddToCart = useCallback(async () => {
    ...
    try {
      ...
      showToast(error?.message || error || "خطا در افزودن به سبد خرید", "error");
    } finally {
  ...
  const handleIncreaseQuantity = useCallback(async () => {
    ...
    try {
      ...
      showToast(error?.message || error || "خطا در به‌روزرسانی سبد خرید", "error");
    } finally {
      setIsCartProcessing(false);
    }
  }, [
  ...
  const handleRemoveFromCart = useCallback(async () => {
    ...
    try {
      ...
      showToast(error?.message || error || "خطا در حذف از سبد خرید", "error");
    } finally {
      setIsCartProcessing(false);
    }
  }, [
